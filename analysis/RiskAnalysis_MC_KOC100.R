...
params <- list(
  ...
  outout.dir = "\\\\by0w5z\\GisData\\External1\\Projects_TS\\current\\Aquatic_Catchment\\processing\\Experiments\\test runs\\reporting_koc100"
)
...
ggplot(d.sed, aes(time, value, col =  reach.id)) +
  ...
  xlab("time [hour]") +
  xlim(c(3050, 3090))
ggsave(file.path(params$outout.dir, "10 reaches around first application (sediment, KOC100).png"))
